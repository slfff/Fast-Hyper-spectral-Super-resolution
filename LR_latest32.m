function hyper = LR_latest32(D_hyper, multi)
...
for i = 1:N/1024
    for j = 1:N/1024
        d_hyper = D_hyper((i-1)*32+1:i*32, (j-1)*32+1:j*32, :);
        d_multi = D_multi((i-1)*32+1:i*32, (j-1)*32+1:j*32, :);
        d_hyper = (hyperConvert2d(d_hyper))';
        d_multi = (hyperConvert2d(d_multi))';
        mean_d_hyper = mean(d_hyper);
        mean_d_multi = mean(d_multi);
        d_hyper = d_hyper - repmat(mean_d_hyper, 1024, 1);
        d_multi = d_multi - repmat(mean_d_multi, 1024, 1);
        
        Lambda = (d_multi' * d_multi)^(-1) * d_multi' * d_hyper;
        
        tmp_multi = multi((i-1)*32*32+1:i*32*32, (j-1)*32*32+1:j*32*32, :);
        tmp_multi = (hyperConvert2d(tmp_multi))';
        tmp_multi = tmp_multi - repmat(mean(tmp_multi), 32*32*32*32, 1);
        tmp_hyper = tmp_multi * Lambda + repmat(mean_d_hyper, 32*32*32*32, 1);
        
        tmp_hyper = hyperConvert3d(tmp_hyper');
        hyper((i-1)*32*32+1:i*32*32, (j-1)*32*32+1:j*32*32, :) = tmp_hyper;
    end
end
end